...
Требования:
•	Класс Solution должен содержать public static поле
drugsController типа DrugsController.
•	Класс Solution должен содержать public static поле isStopped типа boolean.
•	Класс Solution должен содержать private static void 
метод waitAMoment(), который должен ждать 100 мс.
•	Класс Apteka должен реализовывать интерфейс Runnable.
•	Нить Apteka должна работать пока isStopped = false.
•	Нить Apteka должна использовать drugsController для 
закупки случайного лекарства (getRandomDrug) в количестве (getRandomCount).
•	Нить Apteka должна ждать 300мс между закупками, 
используя метод waitAMoment().
•	Класс Person должен реализовывать интерфейс Runnable.
•	Нить Person должна работать пока isStopped = false.
•	Нить Person должна использовать drugsController для 
продажи случайного лекарства (getRandomDrug) в количестве (getRandomCount).
